...
from data.dataset import REDSDataset
from model.model import BasicVSR
from loss.loss import CharbonnierLoss
from utils.utils import resize_sequences
...
val_set = REDSDataset(args.gt_dir,
                      args.lq_dir,
                      args.scale_factor,
                      args.patch_size,
                      args.num_input_frames,
...
for epoch in range(max_epoch):
    model.train()
    # fix SpyNet and EDVR at first 5000 epoch
    if epoch < 5000:
    ...
    with torch.no_grad():
        for idx, data in enumerate(val_loader):
            ...
            lq_sequences = lq_sequences.to('cuda:0')
            pred_sequences = model(lq_sequences)
            lq_sequences = resize_sequences(lq_sequences, (gt_sequences.size(dim=4)))
