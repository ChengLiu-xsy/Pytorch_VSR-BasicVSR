...
def generate_segment_indices(videopath1, videopath2, num_input_frames=10, filename_tmpl='{:08d}.png'):
    ...
    segment1 = [read_image(os.path.join(videopath1, filename_tmpl.format(i))) / 255. for i in
                range(start_frame_idx, end_frame_idx)]
    segment2 = [read_image(os.path.join(videopath2, filename_tmpl.format(i))) / 255. for i in
                range(start_frame_idx, end_frame_idx)]
...
class REDSDataset(Dataset):
    ...
    def __getitem__(self, idx):
        gt_sequence, lq_sequence = generate_segment_indices(self.gt_seq_paths[idx], self.lq_seq_paths[idx],
